function waveleng = envihdrread(hdrfile, sensorID)
...
% if isfield(info,'map_info')
  if isfield(info,'wavelength')
    ...
    if strcmp(sensorID, 'Exvnir')
    waveleng(:,1) = round(line(1:end,1),2);
    waveleng(:,2) = round(waveleng(:,1));
    
    elseif strcmp(sensorID, 'vnir')
    waveleng(:,1) = round(line(1:end-1,1),2);
    waveleng(:,2) = round(waveleng(:,1));   
    end
