...
%%=======================================================
function ReadRaw_Weather(Filepath)

%% Reading weather data from the dataset
folders = GetFolderSet(Filepath);  %read dir of all folders
filename = 'weather';

for i = 1: length(folders)
    
WeatherFolder = fullfile(folders{i},filename);
if(exist(WeatherFolder, 'file')) % check if the folder exist
...
nFile =  GetFolderDir(subFolder{indx(1)}); % read weather folder dir

fullPath = nFile{1}; 

fprintf('Extracting Zip file from %s\n', fullPath);

[DirName,nFile,extent] = fileparts(nFile{1}); % read filename and dir of weather folder
