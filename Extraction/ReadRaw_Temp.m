...
%%=======================================================
function ReadRaw_Temp(Filepath)
...
for i = 1: length(folders)
  ...
  TempFolder = fullfile(folders{i},filename);
  if(exist(TempFolder, 'file')) % check if the folder exist
  ...
  nFile =  GetFolderDir(subFolder{indx(1)}); % read temp folder dir

  fullPath = nFile{1}; 

  fprintf('Extracting Zip file from %s\n', fullPath);

  [DirName,nFile,extent] = fileparts(nFile{1}); % read filename and dir of temp folder
